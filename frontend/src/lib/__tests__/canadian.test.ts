...
describe('contributionRoomUsed', () => {
  ...
  // After the in-place migration every existing person has a NULL income, so "no
  // income-derived limit" must be distinguishable from "a limit of zero" (issue #23).
  it('marks an RRSP row limitKnown:false when the person has no recorded income', () => {
    const events: ContributionEvent[] = [
      { id: 'c1', date: '2025-02-01', accountId: 'a', personId: 'p1', amount: 12000, kind: 'rrsp' },
    ];
    const row = contributionRoomUsed(events, 2025, LIMITS, {}).find((r) => r.kind === 'rrsp')!;
    expect(row.limitKnown).toBe(false);
    expect(row.usedYtd).toBe(12000);
  });

  it('a recorded income (even zero) or stated room makes the limit known', () => {
    const events: ContributionEvent[] = [
      { id: 'c1', date: '2025-02-01', accountId: 'a', personId: 'p1', amount: 100, kind: 'rrsp' },
    ];
    expect(contributionRoomUsed(events, 2025, LIMITS, { p1: 90000 })
      .find((r) => r.kind === 'rrsp')!.limitKnown).toBe(true);
    expect(contributionRoomUsed(events, 2025, LIMITS, { p1: 0 })
      .find((r) => r.kind === 'rrsp')!.limitKnown).toBe(true);
    const stated: StatedRoom[] = [{ personId: 'p1', kind: 'rrsp', amount: 40000 }];
    expect(contributionRoomUsed(events, 2025, LIMITS, {}, stated)
      .find((r) => r.kind === 'rrsp')!.limitKnown).toBe(true);
  });

  it('non-RRSP kinds always have a known limit — TFSA/FHSA caps do not depend on income', () => {
    const events: ContributionEvent[] = [
      { id: 'c1', date: '2025-02-01', accountId: 'a', personId: 'p1', amount: 100, kind: 'tfsa' },
    ];
    expect(contributionRoomUsed(events, 2025, LIMITS, {}).find((r) => r.kind === 'tfsa')!.limitKnown).toBe(true);
  });
});
...
describe('rrspRefundOpportunities', () => {
  ...
  it('a non-earner gets a 0% marginal rate, not the bottom bracket', () => {
    // Below the basic personal amount no tax is paid, so an RRSP deduction refunds nothing —
    // even against a large carry-forward room.
    const stated: StatedRoom[] = [{ personId: 'x', kind: 'rrsp', amount: 50000 }];
    const out = rrspRefundOpportunities(
      [{ id: 'x', name: 'Sam', role: 'adult', grossIncome: 0 }], [], 2026, LIMITS, stated);
    expect(out[0].remaining).toBe(50000);
    expect(out[0].marginalRate).toBe(0);
    expect(out[0].refund).toBe(0);
  });
